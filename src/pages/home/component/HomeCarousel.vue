<template>
  <section class="ibf-container-1200">
    <q-carousel v-model="slide" transition-prev="jump-right" transition-next="jump-left" swipeable animated
      control-color="primary" padding height="430px" class="bg-grey-2">
      <q-carousel-slide v-for="(dataUrl, index) in imageUrl" :key="index" :name="index + 1" class="q-pa-none">
        <q-img :src="dataUrl.img" height="100%" width="100%" fit="fill" spinner-color="primary" spinner-size="82px" />
        <!-- <q-img
        ...
        <div class="absolute-bottom custom-caption q-pa-md">
          <div class="flex justify-between items-center">
            <div>
              <div class="ibf-h9 text-weight-medium">{{ dataUrl.title }}</div>
            ...
            <div>
              <q-btn flat rounded color="grey-10" icon-right="mdi-arrow-right-thin" @click="nextSlide" no-caps
                label="More info" />
            </div>
          </div>
        </div>
    ...
    <div class="row justify-center items-center q-py-sm">
      <q-radio size="35px" v-model="slide" :val="index + 1" v-for="(dataUrl, index) in imageUrl" :key="index" />
    </div>
    <!-- <div class="row justify-center items-center q-py-sm">
      <q-radio
        size="35px"
